...
class C2C(Device):
    '''
    Two Collimating injectors separated by a grounded chamber for
    investigating collimator to collimator transmission
    '''
    ...
    def add_collimators(self):
        self.generate_collimator(0, -self.body_width/2, 0, 1)
        self.generate_collimator(0, self.body_width/2, 180, 2)

    def add_body_ohmics(self):
        W = self.body_width
        L = self.body_length
        pad = self.pad
